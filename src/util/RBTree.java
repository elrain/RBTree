...
public class RBTree<V> {
	...
	public RBTree(Comparator<? super V> comparator) {
		this.comparator = comparator;
	}

	public void add(int key, V v) {
		Node<V> node = root, parent = null;
		while (null != node) {
			int compare = key - node.key;
			if (compare == 0) {
				node.value = v;
				return;
			} else {
...
//				grandparent.parent.left.left.parent = grandparent.parent.left;
	...
	public V get(int key) {
		if (null == nodeForSearch) {
			nodeForSearch = root;
		}
		if (key == nodeForSearch.key) {
			return nodeForSearch.value;
		} else if (key > nodeForSearch.key) {
			nodeForSearch = nodeForSearch.right;
			return get(key);
		} else if (key < nodeForSearch.key) {
			nodeForSearch = nodeForSearch.left;
			return get(key);
		}
